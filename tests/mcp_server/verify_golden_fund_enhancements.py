...
class TestGoldenFundEnhancements(unittest.IsolatedAsyncioTestCase):
    ...
    def test_ckan_search_params(self):
        ...
        with patch.object(self.connector.session, "get") as mock_get:
            ...
            self.connector.search_packages(
                "query", rows=5, filters={"groups": "legal"}, sort="metadata_modified desc"
            )
    ...
    def test_scraper_auth_retry(self, mock_keychain_bridge):
        """Verify that DataScraper retries with credentials on 401."""
        mock_bridge = mock_keychain_bridge.return_value
        mock_bridge.get_credential_for_domain.return_value = MagicMock(
            secret="test_secret", source="macos_keychain"
        )
    ...
    async def test_search_and_ingest(self, mock_ingest, mock_connector):
        ...
        mock_conn.search_packages.return_value = [
            {
                "name": "pkg1",
                "title": "Package 1",
                "resources": [{"format": "CSV", "url": "http://pkg1.csv"}],
            }
        ]
        mock_conn.find_resources_by_format.return_value = [
            {"format": "CSV", "url": "http://pkg1.csv"}
        ]
