...
async def run_diagnostic():
    ...
    print(f"SQL Metadata Match for '{query}': {res.success}, Count: {len(res.data) if res.data else 0}")
    
    print("\n--- Searching Keyword Index (FTS5) ---")
    search = SearchStorage()
    search_res = search.search(query)
    print(f"Keyword Search Match for '{query}': {search_res.success}, Results: {len(search_res.data.get('results', [])) if search_res.data else 0}")
    
    print("\n--- Searching Vector Index (Chroma) ---")
    vector = VectorStorage()
    if vector.enabled:
        vec_res = vector.search(query)
        print(f"Vector Search Match for '{query}': {vec_res.success}, Results: {len(vec_res.data.get('results', [])) if vec_res.data else 0}")
    else:
