...
async def test_model(model_name, test_name, message):
    ...
    try:
        ...
        for line in stderr_output.split('\n'):
            if 'Chat Mode:' in line or 'found.*file' in line or 'Action:' in line or 'Created' in line or 'Modified' in line:
                print(f'  📝 {line}')
...
async def test_dual_models():
    ...
    success1, response1 = await test_model('swe-1.5', 'Working Model', 'Create test_swe.txt with content "swe-1.5 works"')
    
    print('\n' + '-' * 60 + '\n')
    
    # Test 2: deepseek-v3 (second model)
    success2, response2 = await test_model('deepseek-v3', 'DeepSeek V3', 'Create test_deepseek.txt with content "deepseek-v3 works"')
