...
async def main():
    ...
    for tool_name, args in native_tests:
        ...
        try:
            ...
            server, resolved_tool, new_args = dispatcher._resolve_routing(
                tool_name, dict(args), None
            )
            print(
                f"Мапінг: сервер '{server}', інструмент '{resolved_tool}' | Аргументи: {new_args}"
            )

            # Check execution
            res = await dispatcher.resolve_and_dispatch(tool_name, dict(args))
            print(
                f"Виконання '{tool_name}': "
                + (
                    "Успішно ✅"
                    if res.get("success")
                    else f"Помилка ❌ ({res.get('error', 'unknown')})"
                )
            )
        ...
        try:
            ...
            print(
                ...
                + (
                    "Успішно ✅"
                    if res.get("success")
                    else f"Помилка ❌ ({res.get('error', 'Validation error')})"
                )
            )
        except Exception as e:
