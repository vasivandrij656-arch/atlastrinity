...
const BINARY_PATH = path.join(__dirname, '../../vendor/mcp-server-windsurf/.build/arm64-apple-macosx/release/mcp-server-windsurf');
...
async function testModel(modelName, toolName = 'windsurf_chat') {
    ...
    return new Promise((resolve) => {
        ...
        setTimeout(() => {
            ...
            console.log('----------------------------');
            const success = output.includes('Success') || (output.includes('deltaMessage') && output.length > 500) || output.includes('Action: Created/Modified');
            if (success) {
...
async function runAllTests() {
    ...
    results.push({ model: 'windsurf-fast', success: await testModel('windsurf-fast', 'windsurf_chat') });
