...
def main():
    ...
    try:
        resp = requests.post(url, headers=headers, data=make_envelope(payload), stream=True)
    ...
    cascade_url = (
        f"http://127.0.0.1:{port}/exa.language_server_pb.LanguageServerService/StartCascade"
    )
    ...
    try:
        resp = requests.post(cascade_url, headers=headers_proto, data=envelope)
