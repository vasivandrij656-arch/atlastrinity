...
class ParallelOptimizer:
    ...
    def __init__(self, max_parallel: int = 12):  # Increased from 8
        ...
        self.execution_stats = {"parallel_calls": 0, "sequential_calls": 0, "time_saved": 0.0}
        ...
        for batch in batches:
            if len(batch) == 1:
                ...
                batch_results = await asyncio.gather(
                    *[self._execute_single(tool) for tool in batch], return_exceptions=True
                )
    ...
    def get_efficiency_report(self) -> dict[str, Any]:
        ...
        total_calls = (
            self.execution_stats["parallel_calls"] + self.execution_stats["sequential_calls"]
        )
