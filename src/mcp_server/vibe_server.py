...
def _prepare_temp_vibe_home(model_alias: str) -> str:
    ...
    try:
        ...
        toml_lines = [
            ...
            f"timeout_s = {float(get_val(config, 'timeout_s', 1800.0))}",
            "",
        ]
...
async def run_vibe_subprocess(
    ...
    try:
        ...
        result = await _execute_vibe_programmatic(
            prompt=prompt_preview or "No prompt provided",
            cwd=cwd,
            timeout_s=timeout_s,
...
async def _execute_vibe_programmatic(
    prompt: str,
    cwd: str | None,
    timeout_s: float,
    process_env: dict[str, str],
    ctx: Context | None,
    agent_name: str = "auto-approve",
    max_turns: int = 45,
    ...
    try:
        ...
        logger.error(f"[VIBE] Failed to import vibe native modules: {e}")
        return {
            "success": False,
    ...
    try:
        # Apply subprocess environment surgically
        ...
        for attempt in range(MAX_RETRIES):
            ...
            try:
                ...
                with redirect_stderr(err_buf), redirect_stdout(sys.stderr):
                    # Hijack __stdout__ as well for deep-seated prints
                    ...
                    try:
                        ...
                        async def _async_vibe_run() -> str | None:
                            try:
                                ...
                                if hasattr(agent_loop, "telemetry_client"):
        ...
        # Revert environment surgically
        for k, old_v in modified_keys.items():
            if old_v is None:
...
async def vibe_prompt(
    ...
    try:
        ...
        if result.get("success") and result.get("parsed_response"):
            try:
                # Try to load if it's a string, or just use as is if already parsed
                resp_data = result["parsed_response"]
