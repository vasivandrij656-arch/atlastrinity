...
const MACOS_USE_TOOLS: BridgedToolDefinition[] = [
  ...
  },
  {
    ...
    schema: z.object({ text: z.string().describe('The text to type'), pid: pidParam, ...traversalOptions }),
    categories: [ToolCategory.UI_AUTOMATION],
    priority: 1,
  ...
  },
  {
    ...
    schema: z.object({
      ...
      modifierFlags: z.array(z.string()).optional().describe('Modifier keys: "Command", "Shift", "Control", "Option", "Function"'),
      pid: pidParam,
      ...traversalOptions,
    }),
    categories: [ToolCategory.UI_AUTOMATION],
  ...
  // --- App & Window Management ---
  {
    ...
    schema: z.object({ identifier: z.string().describe('App name, bundle ID, or file path'), ...traversalOptions }),
  ...
  },
  {
    ...
    schema: z.object({ pid: z.number().describe('Process ID of the application to traverse'), ...traversalOptions }),
  ...
  },
  {
    ...
    schema: z.object({
      browser: z.string().optional().describe('Browser name (chrome, safari, firefox). Omit to check all.'),
    }),
    categories: [ToolCategory.APP_MANAGEMENT],
    priority: 3,
  ...
  },
  {
    ...
    schema: z.object({
      action: z.enum(['move', 'resize', 'minimize', 'maximize', 'close', 'fullscreen', 'snapshot', 'restore', 'tile', 'list']).describe('Window action'),
      pid: pidParam,
  ...
  },
  {
    ...
    schema: z.object({
      windowName: z.string().optional().describe('Title of the window to close. If omitted, closes front window.'),
    }),
    categories: [ToolCategory.APP_MANAGEMENT],
    priority: 3,
  ...
  // --- Visual Testing / Screenshots / OCR ---
  {
    ...
    description: 'Take screenshots with region selection, multi-monitor support, compression, and OCR',
    backendId: MACOS_USE,
  ...
  // --- System Monitoring ---
  {
    ...
    schema: z.object({
      metric: z.enum(['cpu', 'memory', 'disk', 'network', 'battery', 'all']).describe('Metric to monitor'),
  ...
  },
  {
    ...
    schema: z.object({
      action: z.enum(['volume', 'brightness', 'dark_mode', 'dnd', 'sleep', 'lock', 'info']).describe('System action'),
  ...
  },
  {
    ...
    schema: z.object({
      action: z.enum(['list', 'kill', 'restart', 'monitor', 'priority']).describe('Action to perform'),
  ...
  // --- File & Finder Operations ---
  {
    ...
    schema: z.object({
      ...
      filter: z.string().optional().describe('Filter pattern (e.g., "*.txt", "name contains test")'),
  ...
  },
  {
    ...
    schema: z.object({ metadata: z.boolean().optional().describe('Include file metadata for selected items') }),
    categories: [ToolCategory.FILE_OPS],
    priority: 3,
  ...
  // --- AppleScript ---
  {
    ...
    schema: z.object({
      ...
      template: z.string().optional().describe('Use predefined template (automation, file_ops, system_info)'),
      aiGenerate: z.boolean().optional().describe('Generate AppleScript using AI based on description'),
      description: z.string().optional().describe('Describe what you want to accomplish for AI generation'),
  ...
  // --- Voice Control ---
  {
    ...
    schema: z.object({
      command: z.string().describe('Voice command to execute (e.g., "open Safari", "take screenshot")'),
      language: z.enum(['en-US', 'uk-UA', 'ru-RU', 'de-DE']).optional().describe('Language for voice recognition'),
  ...
  // --- Security ---
  {
    ...
    schema: z.object({
      action: z.enum(['encrypt', 'decrypt', 'encrypt_folder', 'decrypt_folder']).describe('Encryption action'),
  ...
  // --- Permissions ---
  {
    ...
    description: 'Request all necessary macOS permissions (Accessibility, Calendar, Reminders, Notifications)',
    backendId: MACOS_USE,
...
const GOOGLEMAPS_TOOLS: BridgedToolDefinition[] = [
  {
    ...
    schema: z.object({ address: z.string().describe('Address to geocode (e.g., "Kyiv, Ukraine")') }),
  ...
  },
  {
    ...
    schema: z.object({
      ...
      map_type: z.enum(['roadmap', 'satellite', 'terrain', 'hybrid']).optional().describe('Map type'),
    }),
    categories: [ToolCategory.MAPS],
    priority: 3,
...
export function getToolsByCategories(categories: ToolCategoryValue[]): BridgedToolDefinition[] {
  ...
  return ALL_BRIDGED_TOOLS.filter((tool) =>
    tool.categories.some((cat) => categorySet.has(cat)),
  );
}
