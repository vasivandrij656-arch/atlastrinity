...
class PerformanceManager {
    ...
    func getCachedResponse(for key: String, model: String, type: CachedResponse.MessageType) -> String? {
        return cacheQueue.sync {
            guard let cached = responseCache[key],
                  !cached.isExpired,
                  cached.model == model,
                  cached.messageType == type else {
                return nil
            }
            
            // Update access time
            responseCache[key]?.timestamp = Date()
            return cached.response
        }
    }
    
    /// Cache a response with intelligent key generation
    func cacheResponse(_ response: String, for request: String, model: String, type: CachedResponse.MessageType) {
        cacheQueue.async {
    ...
    private func generateCacheKey(for request: String, model: String, type: CachedResponse.MessageType) -> String {
    ...
    private func optimizeMessage(_ message: String) -> String {
        ...
        for phrase in fillerPhrases {
            optimized = optimized.replacingOccurrences(of: "\\b\(phrase)\\b", with: "", options: [.regularExpression, .caseInsensitive])
    ...
    private func calculateComplexity(_ message: String) -> RequestComplexity {
        ...
        let hasCode = message.contains("```") || message.contains("def ") || message.contains("function")
        let hasMultipleSteps = message.lowercased().contains("step") || message.lowercased().contains("then")
