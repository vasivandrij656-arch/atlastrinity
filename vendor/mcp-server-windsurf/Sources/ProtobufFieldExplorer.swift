...
class ProtobufFieldExplorer {
    ...
    private func testFieldSet(fields: [(field: Int, value: Int)], modelUid: String, experimentId: Int) async -> FieldExperiment {
        let startTime = Date()
    ...
    private func sendProto(_ endpoint: String, _ payload: Data) async throws -> Data {
        ...
        guard let httpResp = resp as? HTTPURLResponse, httpResp.statusCode == 200 else {
            throw NSError(domain: "WindsurfMCP", code: 1, userInfo: [NSLocalizedDescriptionKey: "HTTP Error for \(endpoint)"])
        }
...
struct FieldExperiment {
    ...
    init(experimentId: Int, fields: [(field: Int, value: Int)], success: Bool, responseTime: TimeInterval, cascadeId: String? = nil, fileCreated: Bool? = nil, error: String? = nil) {
...
class FieldExperimentAnalyzer {
    static func analyzeResults(_ experiments: [FieldExperiment]) -> AnalysisResult {
        ...
        let successfulFieldSets = successfulExperiments.map { Set($0.fields.map { $0.field }) }
        let failedFieldSets = failedExperiments.map { Set($0.fields.map { $0.field }) }
        ...
        for experiment in experiments {
            for (field, _) in experiment.fields {
                let currentCount = fieldSuccessRates[field, default: 0.0]
    ...
    private static func generateRecommendations(fieldSuccessRates: [Int: Double]) -> [String] {
        ...
        let moderateSuccessFields = fieldSuccessRates.filter { $0.value >= 0.3 && $0.value < 0.7 }.map { $0.key }
        
        if !moderateSuccessFields.isEmpty {
            recommendations.append("Moderate-success fields worth further testing: \(moderateSuccessFields)")
        }
