...
class ErrorRecoveryManager {
    ...
    private func attemptRecovery(for record: ErrorRecord) {
        ...
        fputs("log: [windsurf] Attempting recovery: \(strategy.name) (attempt \(retryCount)/\(strategy.maxRetries))\n", stderr)
    ...
    private func handleConnectionFailure(_ context: ErrorContext) {
        ...
        Task {
            ...
            if let newConnection = await state.ensureConnection() {
                fputs("log: [windsurf] Successfully reconnected to Language Server on port \(newConnection.port)\n", stderr)
                
                // Retry the original operation if possible
                if let originalMessage = context.parameters?["message"] {
                    _ = await handleCascade(message: originalMessage, model: context.parameters?["model"])
                }
            } else {
    ...
    private func handleAPIKeyError(_ context: ErrorContext) {
        ...
        if apiKey.isEmpty {
            fputs("log: [windsurf] ERROR: WINDSURF_API_KEY environment variable is not set\n", stderr)
            fputs("log: [windsurf] Please set it with: export WINDSURF_API_KEY=sk-ws-...\n", stderr)
        } else if !apiKey.hasPrefix("sk-ws-") || apiKey.count <= 20 {
            fputs("log: [windsurf] ERROR: Invalid API key format. Expected format: sk-ws-...\n", stderr)
        } else {
    ...
    private func handleFileSystemError(_ context: ErrorContext) {
        ...
        if !isWritable {
            ...
            fputs("log: [windsurf] Please check file permissions or change to a writable directory\n", stderr)
        }
    }
    ...
    private func generateRecommendations(errorCounts: [String: Int]) -> [String] {
        ...
        if errorCounts["connection_failed", default: 0] > 2 {
            recommendations.append("Multiple connection failures detected. Check if Windsurf IDE is running.")
        }
        
        if errorCounts["cascade_timeout", default: 0] > 2 {
            recommendations.append("Frequent timeouts detected. Consider using simpler requests or different models.")
        }
        
        if errorCounts["api_key_error", default: 0] > 0 {
            recommendations.append("API key issues detected. Verify your WINDSURF_API_KEY environment variable.")
        }
        
        if errorCounts["filesystem_error", default: 0] > 1 {
            recommendations.append("File system errors detected. Check directory permissions and disk space.")
        }
    ...
    private func performCascadeWithTimeout(message: String, model: String?) async throws -> String {
        ...
        return try await withThrowingTaskGroup(of: String.self) { group in
            ...
            group.addTask {
                ...
                throw NSError(domain: "WindsurfMCP", code: 2, userInfo: [NSLocalizedDescriptionKey: "Cascade operation timed out"])
            }
