...
class FileSystemMonitor {
    ...
    func startMonitoring() -> Bool {
        ...
        guard fileDescriptor != -1 else {
            fputs("log: [windsurf] Failed to open directory for monitoring: \(monitoredDirectory.path)\n", stderr)
            return false
        }
        ...
        fputs("log: [windsurf] Started file system monitoring for: \(monitoredDirectory.path)\n", stderr)
        return true
    }
    ...
    private func scanForModifications() {
        ...
        do {
            ...
            for fileName in contents {
                let filePath = (monitoredDirectory.path as NSString).appendingPathComponent(fileName)
...
class ActionPhaseVerifier {
    ...
    init(workspacePath: String) {
        fileMonitor = FileSystemMonitor(directoryPath: workspacePath) { [weak self] event in
    ...
    func stopVerification() -> ActionPhaseResult {
        ...
        fputs("log: [windsurf] Action Phase verification completed: \(detectedEvents.count) events detected\n", stderr)
        return result
    }
    ...
    private func handleFileEvent(_ event: FileSystemMonitor.FileEvent) {
        ...
        let message = "log: [windsurf] Action Phase event: \(event.type.description) - \(event.path)\n"
