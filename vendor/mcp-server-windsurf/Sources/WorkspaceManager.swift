...
class WorkspaceManager {
    ...
    func switchToWorkspace(_ workspaceId: String) -> Bool {
        ...
        fputs("log: [windsurf] Switched to workspace: \(workspaces[workspaceId]?.name ?? "Unknown") (\(workspaceId))\n", stderr)
        return true
    }
    ...
    func detectWorkspaceFromCurrentDirectory() -> WorkspaceContext? {
        ...
        if let existingWorkspace = workspaces.values.first(where: { $0.path == currentPath }) {
            switchToWorkspace(existingWorkspace.id)
            return existingWorkspace
        }
        
        // Auto-detect git repository
        if isGitRepository(currentPath) {
            let repoName = getGitRepositoryName(currentPath) ?? URL(fileURLWithPath: currentPath).lastPathComponent
    ...
    private func getGitRepositoryName(_ path: String) -> String? {
        ...
        do {
            ...
            if let url = String(data: data, encoding: .utf8)?.trimmingCharacters(in: .whitespacesAndNewlines),
               !url.isEmpty {
                return URL(string: url)?.lastPathComponent.replacingOccurrences(of: ".git", with: "")
            }
        } catch {
    ...
    private func detectProjectType(_ path: String) -> String? {
        ...
        if fileManager.fileExists(atPath: (path as NSString).appendingPathComponent("requirements.txt")) ||
           fileManager.fileExists(atPath: (path as NSString).appendingPathComponent("setup.py")) ||
           fileManager.fileExists(atPath: (path as NSString).appendingPathComponent("pyproject.toml")) {
            return "python"
        }
...
struct GitInfo: Codable {
    ...
    static func detect(for path: String) -> GitInfo? {
        ...
        do {
            ...
            guard let remoteUrl = String(data: remoteData, encoding: .utf8)?.trimmingCharacters(in: .whitespacesAndNewlines),
            ...
            return GitInfo(remoteUrl: remoteUrl, branch: branch, commitHash: commitHash, isDirty: isDirty)
...
struct ProjectSettings: Codable {
    ...
    static func detect(for path: String) -> ProjectSettings? {
        ...
        if fileManager.fileExists(atPath: (path as NSString).appendingPathComponent("package.json")) {
            do {
                let packageData = try Data(contentsOf: URL(fileURLWithPath: (path as NSString).appendingPathComponent("package.json")))
                ...
                return ProjectSettings(type: "nodejs", buildCommand: nil, testCommand: nil, dependencies: [])
            }
        }
        ...
        if fileManager.fileExists(atPath: (path as NSString).appendingPathComponent("requirements.txt")) {
            do {
                let requirementsData = try Data(contentsOf: URL(fileURLWithPath: (path as NSString).appendingPathComponent("requirements.txt")))
                ...
                return ProjectSettings(type: "python", buildCommand: nil, testCommand: nil, dependencies: [])
            }
        }
...
extension WorkspaceManager {
    ...
    func handleWorkspaceList() -> String {
        ...
        for workspace in workspaces {
            ...
            let gitStatus = workspace.gitInfo != nil ? "📦" : ""
            ...
            result += "   Last Used: \(workspace.lastUsed.formatted(date: .abbreviated, time: .shortened))\n\n"
        }
